...
pub fn get_line_with_condition<F>(prompt:Option<&str>, condition: F)->String  where F: Fn(&String) -> bool{
...
pub fn get_parsed_line_with_condition<T: std::str::FromStr,F>(prompt:Option<&str>, condition: F)->T where F: Fn(&T) -> bool {
    return match get_line(prompt).parse::<T>() {
